import React, { FC, useEffect, useState, useContext } from "react";
import { ActionsContainer, Icon } from "./Actions.styled";
import { changePhotoLikes } from "../../../../../services/firebase";
import CommentRefContext from "../../../../../context/comment-ref";

const Actions: FC<{
  photoDocId: string;
  currentUserId: string;
  likes: string[];
  onLikesChange: React.Dispatch<React.SetStateAction<string[]>>;
}> = ({ photoDocId, currentUserId, likes, onLikesChange }) => {
  const [isClicking, setIsClicking] = useState(false);
  const [isClicked, setIsClicked] = useState(false);
  const CommentRef = useContext(CommentRefContext);

  const updateLikes = async (isAddLikes: boolean) => {
    await changePhotoLikes(photoDocId, [currentUserId], isAddLikes);
    onLikesChange((prev) => {
      return isAddLikes
        ? [...prev, currentUserId]
        : [...prev.filter((id: string) => id !== currentUserId)];
    });
  };

  useEffect(() => {
    setIsClicked(likes.includes(currentUserId));
  }, [likes, currentUserId]);

  return (
    <ActionsContainer>
      <Icon
        onClick={() => {
          if (!isClicking) {
            updateLikes(!isClicked);
            setIsClicked((prev) => !prev);
            if (!isClicked) {
              setIsClicking(true);
            }
          }
        }}
        isClicked={isClicked}
        isClicking={isClicking}
        onAnimationEnd={() => setIsClicking(false)}
        src="data:image/png;base64,"
      />
      <Icon
        onClick={() => CommentRef.current.focus()}
        isClicked={false}
        isClicking={false}
        src="data:image/png;base64,"
      />
    </ActionsContainer>
  );
};

export default Actions;
